---
title: "Question 2 - Entrepreneurship"
author: "Katz et al."
date: "5/22/2021"
...
rf_mod <- randomForest(formula = Q2e ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Interest in Entrepreneurship")
...
## Option 2: Ordinal forest instead of regression forest for Q16l
...
ord_rf_df %>% select(Q2e)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q2e", 
                     data = ord_rf_df, 
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Interest in Entrepreneurship")
...
```{r}
get_prior(ordered(Q2e) ~ Q4b + Q4g + Q16h + Q15l + Q15a + Q4l + Q3d + Q16k, 
          data = train_df, family = cumulative("logit"))
```


```{r}
priors1 <- c(set_prior("normal(0, 1)", class = "b", coef = "Q4b"),
             set_prior("normal(0, 1)", class = "b", coef = "Q4g"),
             set_prior("normal(0, 1)", class = "b", coef = "Q16h"),
             set_prior("normal(0, 1)", class = "b", coef = "Q15l"),
             set_prior("normal(0, 1)", class = "b", coef = "Q15a"),
             set_prior("normal(0, 1)", class = "b", coef = "Q4l"),
             set_prior("normal(0, 1)", class = "b", coef = "Q3d"),
             set_prior("normal(0, 1)", class = "b", coef = "Q16k"))
...
```{r}
# no priors
mod1_ord_Q2e <- brm(formula = ordered(Q2e) ~ Q4b + Q4g + Q16h + Q15l + Q15a + Q4l + Q3d + Q16k,
                     data = train_df,
...
```{r}
path <- "G:/My Drive/AK Faculty/Research/Projects/project students and climate change/analysis/question_2/"

file_name <- "mod1_ord_Q2e.rds"

mod1_ord_Q2e %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q2e <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q2e)
```


```{r}
stanplot(mod1_ord_Q2e, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
```{r}
param_plot <- mod1_ord_Q2e %>%
  spread_draws(`b_.*`, regex = TRUE) %>%
  pivot_longer(cols = b_Q4b:b_Q16k, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  geom_vline(xintercept = 0, color = "black", linetype = "dashed") +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Parameter Estimate Densities for Orginal Regression of Entrepreneurship Interest",
       x = "Parameter Estimate",
       y = "Survey Item") +
  theme_light() +
...
mod1_ord_Q2e %>%
  ...
  labs(title = "Intercept Estimates for Ordinal Regression with Entrepreneurship Interest",
       x = "Parameter Estimate",
       y = "Cluster cutoff point") +
  theme(plot.title = element_text(hjust=0.5))
...
```{r}
tab_model(mod1_ord_Q2e)
```



#### Ordinal logistic regression with monotonic effects


```{r}
# no priors
mod1_ord_mono_Q2e <- brm(formula = ordered(Q2e) ~ mo(Q4b) + mo(Q4g) + mo(Q16h) + mo(Q15l) + mo(Q15a) + mo(Q4l) + Q3d + mo(Q16k),
                     data = train_df,
                     family = cumulative("logit"),
                     cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q2e.rds"

mod1_ord_mono_Q2e %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q2e <- read_rds(file = paste0(path, file_name))
