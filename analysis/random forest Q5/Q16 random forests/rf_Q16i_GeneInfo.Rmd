---
title: "Random Forest - Q16i - Gene Info"
author: "Katz et al."
date: "5/19/2021"
...
### Option 1: Regular regression forest
...
rf_mod <- randomForest(formula = Q16i ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Working on Gene Info to Help People")
...
ord_rf_df %>% select(Q16i)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16i", 
                     data = ord_rf_df, 
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Using Genetic Info to Help People")
...
# Model interest using results from ordinal forest
...
```{r}
get_prior(formula = ordered(Q16i) ~ major + Q2f + Q24f + Q7l_wt_sum + Q7h_wt_sum + Q3f + Q18g + Q7e_wt_sum, 
...
mod1_ord_Q16i <- brm(formula = ordered(Q16i) ~ major + Q2f + Q24f + Q7l_wt_sum + Q7h_wt_sum + Q3f + Q18g + Q7e_wt_sum,
                     data = train_df,
...
file_name <- "mod1_ord_Q16i.rds"

mod1_ord_Q16i %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16i <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16i)
```


```{r}
stanplot(mod1_ord_Q16i, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
mod1_ord_mono_Q16i <- brm(formula = ordered(Q16i) ~ major + mo(Q2f) + mo(Q24f) + Q7l_wt_sum + 
                            Q7h_wt_sum + Q3f + mo(Q18g) + Q7e_wt_sum,
                     data = train_df,
                     family = cumulative("logit"),
                     cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16i.rds"

mod1_ord_mono_Q16i %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16i <- read_rds(file = paste0(path, file_name))
