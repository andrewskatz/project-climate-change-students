---
title: "Random Forest - Q16L - Clean Water"
author: "Katz et al."
date: "5/19/2021"
...
rf_mod <- randomForest(formula = Q16l ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Technologies to Provide Clean Drinking Water")
...
```{r}
ord_rf_df %>% select(Q16l)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16l", 
                     data = ord_rf_df, 
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Technologies to Provide Clean Drinking Water")
...
# Create ordinal logistic regression model with Q16l (address climate change in career) and the following predictors:
Q18c
Q29
Q4l
Q17f
...
## Option 1: Ordinal regression with continuous effects

```{r}
# no priors
mod1_ord_Q16l <- brm(formula = ordered(Q16l) ~ Q18c + Q29 + Q4l + Q17f + Q17e + Q18e + Q18j + Q4p,
                  data = train_df,
...
file_name <- "mod1_ord_Q16l.rds"

mod1_ord_demog %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16l <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16l)
```


```{r}
stanplot(mod1_ord_Q16l, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
## Opition 2: Ordinal regression with monotonic effects



```{r}
# no priors
mod1_ord_mono_Q16l <- brm(formula = ordered(Q16l) ~ major + mo(Q18c) + mo(Q4l) + mo(Q17f) + 
                       mo(Q17e) + mo(Q18e) + mo(Q18j) + mo(Q4p),
                  data = train_df,
                  family = cumulative("logit"),
                  cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16l.rds"

mod1_ord_mono_Q16l %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16l <- read_rds(file = paste0(path, file_name))
