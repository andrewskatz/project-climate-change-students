---
title: "Random Forest - Q16 - Energy Efficiency"
author: "Katz et al."
date: "5/19/2021"
...
rf_mod <- randomForest(formula = Q16b ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Energy Efficiency")
...
ord_rf_df %>% select(Q16b)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16b", 
                     data = ord_rf_df, 
...
# Create Ordinal logistic regression model with Q16b (address energy efficiency in career) and the following predictors:
...
```{r}
# no priors
mod1_ord_Q16b <- brm(formula = ordered(Q16b) ~ Q18c + Q18e + Q12defghi_norm + Q17f + 
                       Q18i + Q14c + major + Q24e,
                     data = train_df,
                     prior = priors1,
...
file_name <- "mod1_ord_Q16b.rds"

mod1_ord_Q16b %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16b <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16b)
```


```{r}
stanplot(mod1_ord_Q16b, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
mod1_ord_mono_Q16b <- brm(formula = ordered(Q16b) ~ mo(Q18c) + mo(Q18e) + Q12defghi_norm + 
                            mo(Q17f) + mo(Q18i) + mo(Q14c) + major + mo(Q24e),
                     data = train_df,
                     family = cumulative("logit"),
                     cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16b.rds"

mod1_ord_mono_Q16b %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16b <- read_rds(file = paste0(path, file_name))
