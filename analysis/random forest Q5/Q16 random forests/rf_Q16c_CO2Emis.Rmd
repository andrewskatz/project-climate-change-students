---
title: "Random Forest - Q16 - carbon dioxide"
author: "Katz et al."
date: "5/19/2021"
...
rf_mod <- randomForest(formula = Q16c ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
ord_rf_df %>% select(Q16c)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16c", 
                     data = ord_rf_df, 
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Carbon Dioxide Emissions")
...
```{r}
# no priors
mod1_ord_Q16c <- brm(formula = ordered(Q16c) ~ Q18c + Q20d + Q18k + Q18e + 
                       Q17f + Q18j + Q18i + Q19h,
                     data = train_df,
                     prior = priors1,
...
file_name <- "mod1_ord_Q16c.rds"

mod1_ord_Q16c %>% write_rds(file = paste0(path, file_name))
```
...
```{r}
stanplot(mod1_ord_Q16c, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
