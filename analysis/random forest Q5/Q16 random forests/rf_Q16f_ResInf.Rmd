---
title: "Random Forest - Q16 - Resilient Infrastructure"
author: "Katz et al."
date: "5/19/2021"
...
rf_mod <- randomForest(formula = Q16f ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Infrastructure Resilient to Extreme Weather")
...
```{r}
ord_rf_df %>% select(Q16f)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16f", 
                     data = ord_rf_df, 
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Infrastructure Resilient to Extreme Weather")
...
## Create logistic regression model with Q16f (address climate change in career) and the following predictors:
Q17e
Q7e_wt_sum
Q18c
Q17f
...
```{r}
# no priors
mod1_ord_Q16f <- brm(formula = ordered(Q16f) ~ major + Q17e + Q7e_wt_sum + Q18c + 
                       Q17f + Q18j + Q17j + Q12i + Q18e,
                  data = train_df,
                  prior = priors1,
...
file_name <- "mod1_ord_Q16f.rds"

mod1_ord_Q16f %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16f <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16f)
```


```{r}
stanplot(mod1_ord_Q16f, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
## Option 2: Ordinal logistic regression with ordinal predictors
...
```{r}
# no priors
mod1_ord_mono_Q16f <- brm(formula = ordered(Q16f) ~ major + mo(Q17e) + Q7e_wt_sum + mo(Q18c) + 
                       mo(Q17f) + mo(Q18j) + mo(Q17j) + mo(Q12i) + mo(Q18e),
                  data = train_df,
                  family = cumulative("logit"),
                  cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16f.rds"

mod1_ord_mono_Q16f %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16f <- read_rds(file = paste0(path, file_name))
...
summary(mod1_ord_mono_Q16f)

# coef(mod1_ord_mono_Q16f)
fixef(mod1_ord_mono_Q16f)

```

```{r}
get_variables(mod1_ord_mono_Q16f)
```


```{r}
stanplot(mod1_ord_mono_Q16f, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
  theme(axis.text.y = element_text(hjust = 0))
```
...
### Method using bootstrap regression
