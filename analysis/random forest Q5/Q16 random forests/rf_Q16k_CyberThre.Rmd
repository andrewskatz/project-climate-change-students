---
title: "Random Forest - Q16k - Cyber Threats"
author: "Katz et al."
date: "5/19/2021"
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Protecting Against Cyber Threats")
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Protecting Against Cyber Threats")
...
```{r}
get_prior(ordered(Q16k) ~ major + Q18g + Q18f + Q17b + Q24f + Q2f + Q7p_wt_sum + Q18l, 
...
```{r}
# no priors
mod1_ord_Q16k <- brm(formula = ordered(Q16k) ~ major + Q18g + Q18f + Q17b + Q24f + Q2f + Q7p_wt_sum + Q18l,
                     data = train_df,
...
file_name <- "mod1_ord_Q16k.rds"

mod1_ord_Q16k %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16k <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16k)
```


```{r}
stanplot(mod1_ord_Q16k, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
```{r}
# no priors
mod1_ord_mono_Q16k <- brm(formula = ordered(Q16k) ~ major + mo(Q18g) + mo(Q18f) + mo(Q17b) + 
                            mo(Q24f) + Q2f + Q7p_wt_sum + mo(Q18l),
                     data = train_df,
                     family = cumulative("logit"),
                     cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16k.rds"

mod1_ord_mono_Q16k %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16k <- read_rds(file = paste0(path, file_name))
