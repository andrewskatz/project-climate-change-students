---
title: "Random Forest - Q16 - Climate Change Legislation"
author: "Katz et al."
date: "5/19/2021"
...
rf_mod <- randomForest(formula = Q16g ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Working on Climate Change Legislation")
...
```{r}
ord_rf_df %>% select(Q16g)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16g", 
                     data = ord_rf_df, 
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Working to Create Climate Change Legislation")
...
# Create ordinal regression model with Q16g (working to create climate change legislation) and the following predictors:
...
```{r}
get_prior(ordered(Q16g) ~ Q18c + Q20d + Q18k + Q28_social_norm + Q4l + Q18l + Q17e + Q18d, 
          data = train_df, family = cumulative("logit"))
```


```{r}
priors1 <- c(set_prior("normal(0, 1)", class = "b", coef = "Q18c"),
             set_prior("normal(0, 1)", class = "b", coef = "Q20d"),
             set_prior("normal(0, 1)", class = "b", coef = "Q18k"),
             set_prior("normal(0, 1)", class = "b", coef = "Q28_social_norm"),
             set_prior("normal(0, 1)", class = "b", coef = "Q4l"),
             set_prior("normal(0, 1)", class = "b", coef = "Q18l"),
             set_prior("normal(0, 1)", class = "b", coef = "Q17e"),
             set_prior("normal(0, 1)", class = "b", coef = "Q18d"))
...
mod1_ord_Q16g <- brm(formula = ordered(Q16g) ~ Q18c + Q20d + Q18k + Q28_social_norm + Q4l + Q18l + Q17e + Q18d,
                     data = train_df, prior = priors1,
...
file_name <- "mod1_ord_Q16g.rds"

mod1_ord_Q16g %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16g <- read_rds(file = paste0(path, file_name))
...
```{r}
# no priors
mod1_ord_mono_Q16g <- brm(formula = ordered(Q16g) ~ mo(Q18c) + mo(Q20d) + mo(Q18k) + Q28_social_norm + mo(Q4l) + 
                            mo(Q18l) + mo(Q17e) + mo(Q18d),
                     data = train_df,
                     family = cumulative("logit"),
                     cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16g.rds"

mod1_ord_mono_Q16g %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16g <- read_rds(file = paste0(path, file_name))
...
# Old school logistic regression method
