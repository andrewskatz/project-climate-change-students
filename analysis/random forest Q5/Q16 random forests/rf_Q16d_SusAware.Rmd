---
title: "Random Forest - Q16d - Sustainability Awareness"
author: "Katz et al."
date: "5/19/2021"
...
rf_mod <- randomForest(formula = Q16d ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Raising Sustainability Awareness")
...
ord_rf_df %>% select(Q16d)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16d", 
                     data = ord_rf_df, 
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Spreading Sustainability Awareness")
...
## Create ordinal regression model with Q16d (address climate change in career) and the following predictors:
...
```{r}
# no priors
mod1_ord_Q16d <- brm(formula = ordered(Q16d) ~ Q18c + Q20d + Q18e + Q18k + 
                       Q17f + Q17e + Q18j + Q4p,
                  data = train_df,
                  prior = priors1,
...
file_name <- "mod1_ord_Q16d.rds"

mod1_ord_Q16d %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16d <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16d)
```


```{r}
stanplot(mod1_ord_Q16d, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
## Option 2: Ordinal regression model with ordinal predictors



```{r}
# no priors
mod1_ord_mono_Q16d <- brm(formula = ordered(Q16d) ~ mo(Q18c) + mo(Q20d) + mo(Q18e) + mo(Q18k) + 
                       mo(Q17f) + mo(Q17e) + mo(Q18j) + mo(Q4p),
                  data = train_df,
                  family = cumulative("logit"),
                  cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16d.rds"

mod1_ord_mono_Q16d %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16d <- read_rds(file = paste0(path, file_name))
...
summary(mod1_ord_mono_Q16d)

# coef(mod1_ord_mono_Q16d)
fixef(mod1_ord_mono_Q16d)

```

```{r}
get_variables(mod1_ord_mono_Q16d)
```


```{r}
stanplot(mod1_ord_mono_Q16d, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
  theme(axis.text.y = element_text(hjust = 0))
```
