---
title: "Random Forest - Q16 - Energy Consumption"
author: "Katz et al."
date: "5/19/2021"
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Energy Consumption")
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Career Interest in Energy Conservation")
...
# Create Ordinal logistic regression model with Q16a (address energy conservation in career) and the following predictors:
...
```{r}
# weak priors
mod1_ord_Q16a <- brm(formula = ordered(Q16a) ~ Q18c + Q20d + Q18e + Q17f + 
                       Q18i + Q12defghi_norm + Q18k + Q18a,
                     data = train_df,
                     prior = priors1,
...
file_name <- "mod1_ord_Q16a.rds"

mod1_ord_Q16a %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16a <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16a)
```


```{r}
stanplot(mod1_ord_Q16a, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
mod1_ord_mono_Q16a <- brm(formula = ordered(Q16a) ~ mo(Q18c) + mo(Q20d) + mo(Q18e) + mo(Q17f) + mo(Q18i) + Q12defghi_norm + mo(Q18k) + mo(Q18a),
                     data = train_df,
                     family = cumulative("logit"),
                     cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16a.rds"

mod1_ord_mono_Q16a %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16a <- read_rds(file = paste0(path, file_name))
...
## Old school method: Create logistic regression model with Q5d (address climate change in career) and the following predictors:
Q20d
Q18c
Q18k
