---
title: "Random Forest - Q16j - Bio Attacks"
author: "Katz et al."
date: "5/19/2021"
...
rf_mod <- randomForest(formula = Q16j ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
...
# for regular random forest model
#row.names(imp)
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Countering Biological Attacks")
...
```{r}
ord_rf_df %>% select(Q16j)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "Q16j", 
                     data = ord_rf_df, 
...
## Create logistic regression model with Q16j (address biological attacks in career) and the following predictors:
...
```{r}
get_prior(ordered(Q16j) ~ major + Q2f + Q24f + Q18g + Q7h_wt_sum + Q7l_wt_sum + Q7p_wt_sum + Q3f, 
...
```{r}
# Q16j probably already ordered from the ordinal forest above. If not, might need to order

mod1_ord_Q16j <- brm(formula = ordered(Q16j) ~ major + Q2f + Q24f + Q18g + Q7h_wt_sum + Q7l_wt_sum + Q7p_wt_sum + Q3f,
                     data = train_df,
...
file_name <- "mod1_ord_Q16j.rds"

mod1_ord_Q16j %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_Q16j <- read_rds(file = paste0(path, file_name))
...
```{r}
get_variables(mod1_ord_Q16j)
```


```{r}
stanplot(mod1_ord_Q16j, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
```{r}
param_plot <- mod1_ord_Q16j %>%
  spread_draws(`b_.*`, regex = TRUE) %>%
  pivot_longer(cols = b_majorEle:b_Q18l, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Parameter Estimates for Orginal Regression of Cybersecurity Interest",
       x = "Parameter Estimate",
       y = "Cluster cutoff point") +
  theme_light() +
...
```{r}
tab_model(mod1_ord_Q16j)
```



## Option 2: Ordinal logistic regression with ordinal predcitors


```{r}
# Q16j probably already ordered from the ordinal forest above. If not, might need to order

mod1_ord_mono_Q16j <- brm(formula = ordered(Q16j) ~ major + mo(Q2f) + mo(Q24f) + mo(Q18g) + 
                            Q7h_wt_sum + Q7l_wt_sum + Q7p_wt_sum + mo(Q3f),
                     data = train_df,
                     family = cumulative("logit"),
                     cores = parallel::detectCores())
...
file_name <- "mod1_ord_mono_Q16j.rds"

mod1_ord_mono_Q16j %>% write_rds(file = paste0(path, file_name))
```

```{r}

mod1_ord_mono_Q16j <- read_rds(file = paste0(path, file_name))
...
Old school bootstrapped regression approach
