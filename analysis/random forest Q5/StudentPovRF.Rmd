...
Q5_removals <- c("Q5a", "Q5b", "Q5d", "Q5e", "Q5f", "Q5g", "Q5h", "Q5i", "Q5j")
...
Create random forest for Q5c with tidymodels framework
```{r}
...
q5c_split <- train_df %>% 
  initial_split(strata = Q5c)

q5c_train <- training(q5c_split) # 2440 x 288
q5c_test <- testing(q5c_split) # 812 x 288
...
# rf_mod <- randomForest(formula = Q5c ~ ., 
#                        data = q5c_train,
#                        ntree = 1000,
#                        proximity = TRUE,
...
p <- featureImportance %>% 
  ...
  ggtitle("Random Forest Variable Importance for Career Interest in Poverty and Wealth Distribution")
