---
title: "Temporal Beliefs - Academic Experiecnes - Imputed"
author: "Katz et al."
date: "4/14/2021"
...
mod1_ord_acad_tri_inf <- brm(formula = ordered(cluster_time_rank) ~ mo(Q7_env_tally_sum),
            data = cl_df,
            family = cumulative("logit"),
            cores = parallel::detectCores())
...
file_name <- "mod1_ord_acad_tri_inf.rds"

mod1_ord_acad_tri_inf %>% write_rds(file = file_name)
...
```{r}
mod1_ord_transformed <- ggs(mod1_ord_acad_tri_inf) # suggestion from https://www.rensvandeschoot.com/brms-wambs/
```

```{r}
# analyze trace plot
stanplot(mod1_ord_acad_tri_inf, type = "trace")
```



```{r}
summary(mod1_ord_acad_tri_inf)
```


```{r}
# coef(mod1_ord_acad_tri_inf)
```



```{r}
stanplot(mod1_ord_acad_tri_inf, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
```{r}
get_variables(mod1_ord_acad_tri_inf)

mod1_ord_acad_tri_inf %>%
...
mod2_ord_acad_tri_inf <- brm(formula = ordered(cluster_time_rank) ~ Q7_env_tally_sum,
            data = cl_df,
            prior = priors2_ac,
            family = cumulative("logit"),
            cores = parallel::detectCores())
...
file_name <- "mod2_ord_acad_tri_inf.rds"

mod2_ord_acad_tri_inf %>% write_rds(file = paste0(path, file_name))
```
...
```{r}
summary(mod2_ord_acad_tri_inf)
```


```{r}
stanplot(mod2_ord_acad_tri_inf, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
  theme(axis.text.y = element_text(hjust = 0))
...
```{r}
mod3_ord_acad_tri_inf <- brm(formula = ordered(cluster_time_rank) ~ mo(help_people) + mo(cont_iss) + mo(env_exp) + mo(int_ser_exp) + mo(dev_country_exp) + mo(work_exp) + mo(res_exp),
            data = cl_df,
            family = cumulative("logit"),
            cores = parallel::detectCores())
...
file_name <- "mod3_ord_acad_tri_inf.rds"

mod3_ord_acad_tri_inf %>% write_rds(file = paste0(path, file_name))
```
...
mod4_ord_acad_tri_inf <- brm(formula = ordered(cluster_time_rank) ~ mo(help_people) + mo(cont_iss) + mo(env_exp) + mo(int_ser_exp) + mo(dev_country_exp) + mo(work_exp) + mo(res_exp) + Q7_env_tally_sum,
            data = cl_df,
            family = cumulative("logit"),
            cores = parallel::detectCores())
...
file_name <- "mod4_ord_acad_tri_inf.rds"

mod4_ord_acad_tri_inf %>% write_rds(file = paste0(path, file_name))
```
...
mod4_ord_acad_tri_inf <- read_rds(file = paste0(path, file_name))

```


```{r}
summary(mod4_ord_acad_tri_inf)
```

```{r}
mod4_ord_acad_tri_inf %>%
  spread_draws(`b_.*`, regex = TRUE) %>%
  pivot_longer(cols = `b_Intercept[1]`:b_Q7_env_tally_sum, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Intercept Estimates for Orginal Regression with Spatiotemporal Belief Pattern",
       x = "Parameter Estimate",
       y = "Cluster cutoff point") +
...
mod5_ord_acad_tri_inf <- brm(formula = ordered(cluster_time_rank) ~ mo(help_people) + mo(cont_iss) + mo(env_exp) + mo(int_ser_exp) + mo(dev_country_exp) + mo(work_exp) + mo(res_exp) + (1 + Q7_env_tally_sum | major),
            data = cl_df,
            family = cumulative("logit"),
            cores = parallel::detectCores())
...
file_name <- "mod5_ord_acad_tri_inf.rds"

mod5_ord_acad_tri_inf %>% write_rds(file = paste0(path, file_name))
```
...
mod5_ord_acad_tri_inf <- read_rds(file = paste0(path, file_name))
```
...
summary(mod5_ord_acad_tri_inf)

```

```{r}
get_variables(mod5_ord_acad_tri_inf)
```
...
plot(mod5_ord_acad_tri_inf, pars = "^b")
...
mod5_ord_acad_tri_inf %>%
  spread_draws(`b.*`, regex = TRUE) %>%
  pivot_longer(cols = bsp_mohelp_people:bsp_mores_exp, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Intercept Estimates for Orginal Regression with Spatiotemporal Belief Pattern",
       x = "Parameter Estimate",
...
mod5_ord_acad_tri_inf %>%
  spread_draws(`b.*`, regex = TRUE) %>%
  pivot_longer(cols = `b_Intercept[1]`:`b_Intercept[7]`, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Intercept Estimates for Orginal Regression with Spatiotemporal Belief Pattern",
       x = "Parameter Estimate",
...
mod6_ord_acad_tri_inf <- brm(formula = ordered(cluster_time_rank) ~ mo(help_people) + mo(cont_iss) + mo(env_exp) + mo(int_ser_exp) + mo(dev_country_exp) + mo(work_exp) + mo(res_exp) + Q9a + Q9b + Q9c + (1 + Q7_env_tally_sum | major),
            data = cl_df,
            family = cumulative("logit"),
            cores = parallel::detectCores())
...
file_name <- "mod6_ord_acad_tri_inf.rds"

mod6_ord_acad_tri_inf %>% write_rds(file = paste0(path, file_name))
```

```{r}
mod6_ord_acad_tri_inf <- read_rds("mod6_ord_acad_tri_inf.rds")

```


```{r}
stanplot(mod6_ord_acad_tri_inf, pars = c("^r_", "^b_", "simo_", "^sd_")) +
  theme_light() +
  theme(axis.text.y = element_text(hjust = 0))
```

```{r}
get_variables(mod6_ord_acad_tri_inf)
```

```{r}

mod6_ord_acad_tri_inf %>%
  spread_draws(`b.*`, regex = TRUE) %>%
  pivot_longer(cols = bsp_mohelp_people:bsp_mores_exp, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Intercept Estimates for Ordinal Regression with Spatiotemporal Belief Pattern",
       x = "Parameter Estimate",
       y = "Cluster cutoff point") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  theme(plot.title = element_text(hjust=0.5, size = 10))
...
mod6_ord_acad_tri_inf %>%
  ...
  pivot_longer(cols = -one_of(".chain", ".iteration", ".draw"), names_to = "parameter", values_to = "parameter_estimate") %>% 
...
mod6_ord_acad_tri_inf %>%
  spread_draws(`r.*`, regex = TRUE) %>%
  pivot_longer(cols = -one_of(".chain", ".iteration", ".draw"), names_to = "parameter", values_to = "parameter_estimate") %>% 
  filter(str_detect(parameter, "env")) %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Intercept Estimates for Ordinal Regression with Spatiotemporal Belief Pattern",
       x = "Parameter Estimate",
...
mod7_lin_acad_tri_inf <- brm(formula = cluster_time_rank ~ mo(help_people) + mo(cont_iss) + mo(env_exp) + mo(int_ser_exp) + mo(dev_country_exp) + mo(work_exp) + mo(res_exp) + Q9a + Q9b + Q9c + (1 + Q7_env_tally_sum | major),
            data = cl_df,
            family = gaussian(),
            cores = parallel::detectCores())
...
file_name <- "mod7_lin_acad_tri_inf.rds"

mod7_lin_acad_tri_inf %>% write_rds(file = file_name)
...
mod7_lin_acad_tri_inf <- read_rds("mod7_lin_acad.rds")
...
```{r}
stanplot(mod7_lin_acad_tri_inf, pars = c("^r_", "^b_", "simo_", "^sd_")) +
  theme_light() +
  theme(axis.text.y = element_text(hjust = 0))
```

```{r}
summary(mod7_lin_acad_tri_inf)
```

```{r}
# tab_model(mod7_lin_acad_tri_inf)
cl_df %>% count(cluster_time_rank)
```


```{r}
pp_check(object = mod7_lin_acad_tri_inf, type = "ecdf_overlay")
```
...
loo(mod6_ord_acad_tri_inf, mod7_lin_acad_tri_inf)
...
mod7_lin_acad_tri_inf_mi <- brm_multiple(formula = cluster_time_rank ~ mo(help_people) + mo(cont_iss) + mo(env_exp) + mo(int_ser_exp) + mo(dev_country_exp) + mo(work_exp) + mo(res_exp) + Q9a + Q9b + Q9c + (1 + Q7_env_tally_sum | major),
            data = test_mids,
            family = gaussian(),
            cores = parallel::detectCores())
...
file_name <- "mod7_lin_acad_tri_inf_mi.rds"

mod7_lin_acad_tri_inf_mi %>% write_rds(file = file_name)
...
mod8_lin_acad_tri_inf <- brm(formula = cluster_time_rank ~ mo(help_people) + mo(cont_iss) + mo(env_exp) + mo(int_ser_exp) + mo(dev_country_exp) + mo(work_exp) + mo(res_exp) + Q9a + Q9b + Q9c + Q7_env_tally_sum,
            data = cl_df,
            family = gaussian(),
            cores = parallel::detectCores())
...
file_name <- "mod8_lin_acad_tri_inf.rds"

mod8_lin_acad_tri_inf %>% write_rds(file = file_name)

```

```{r}
file_name <- "mod8_lin_acad_tri_inf.rds"
mod8_lin_acad_tri_inf <- read_rds(file = file_name)

```


```{r}
summary(mod8_lin_acad_tri_inf)

```


```{r}
tab_model(mod8_lin_acad_tri_inf)
```
...
```{r}
get_variables(mod8_lin_acad_tri_inf)
...
```{r}
stanplot(mod8_lin_acad_tri_inf, pars = c("^r_", "^b", "simo_", "^sd_")) +
  theme_light() +
  theme(axis.text.y = element_text(hjust = 0))
...
mod8_lin_acad_tri_inf %>%
  spread_draws(`b.*`, regex = TRUE) %>%
  pivot_longer(cols = b_Q9a:bsp_mores_exp, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Intercept Estimates for Linear Regression with Spatiotemporal Belief Pattern",
       x = "Parameter Estimate",
       y = "Cluster cutoff point") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  theme(plot.title = element_text(hjust=0.5, size = 10))
...
```{r}
plot_mono_est <- function(model, plot_title){
  model %>%
    spread_draws(`sim.*`, regex = TRUE) %>%
    pivot_longer(cols = -one_of(".chain", ".iteration", ".draw"), names_to = "parameter", values_to = "parameter_estimate") %>% 
    ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
    stat_halfeye(.width = c(.9, .5)) +
...
```{r}
plot_mono_est(mod8_lin_acad_tri_inf, "Linear regression estimates for mod8_lin")

```


```{r}
plot_mono_est <- function(model){}
mod8_lin_acad_tri_inf %>%
  spread_draws(`sim.*`, regex = TRUE) %>%
  pivot_longer(cols = -one_of(".chain", ".iteration", ".draw"), names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
...
```{r}
mod9_lin_acad_tri_inf <- brm(formula = cluster_time_rank ~ help_people + cont_iss + env_exp + int_ser_exp + dev_country_exp + work_exp + res_exp + Q9a + Q9b + Q9c + Q7_env_tally_sum,
            data = cl_df,
            family = gaussian(),
            cores = parallel::detectCores())
```
...
file_name <- "mod9_lin_acad_tri_inf.rds"

mod9_lin_acad_tri_inf %>% write_rds(file = file_name)
...
```{r}
summary(mod9_lin_acad_tri_inf)
```


```{r}
# tab_model(mod9_lin_acad_tri_inf)
```
...
mod9_lin_acad_tri_inf_mi <- brm_multiple(formula = cluster_time_rank_rev ~ help_people + cont_iss + env_exp + int_ser_exp + dev_country_exp + work_exp + res_exp + Q9a + Q9b + Q9c + Q7_env_tally_sum,
            data = test_mids,
            family = gaussian(),
            cores = parallel::detectCores())
```
...
```{r}
summary(mod9_lin_acad_tri_inf_mi)
```


```{r}
# tab_model(mod9_lin_acad_tri_inf_mi)
```
...
mod10_lin_acad_tri_inf <- brm(formula = cluster_time_rank ~ Q6a + Q6d + Q6e + Q6i + Q6j + Q8j+ Q8n + Q9a + Q9b + Q9c + Q7_env_tally_sum,
            data = cl_df,
            family = gaussian(),
            cores = parallel::detectCores())
```
...
file_name <- "mod10_lin_acad_tri_inf.rds"

mod10_lin_acad_tri_inf %>% write_rds(file = file_name)
...
```{r}
summary(mod10_lin_acad_tri_inf)
```

```{r}
tab_model(mod10_lin_acad_tri_inf)
```
...
mod11_lin_acad_tri_inf <- brm(formula = cluster_time_rank ~ Q6a + Q6d + Q6e + Q6i + Q6j + Q8j+ Q8n + Q9a + Q9b + Q9c + Q7_env_tally_sum + major,
            data = cl_df,
            family = gaussian(),
            cores = parallel::detectCores())
```
...
file_name <- "mod11_lin_acad_tri_inf.rds"

mod11_lin_acad_tri_inf %>% write_rds(file = file_name)
...
```{r}
summary(mod11_lin_acad_tri_inf)
```

```{r}
tab_model(mod11_lin_acad_tri_inf)
```
