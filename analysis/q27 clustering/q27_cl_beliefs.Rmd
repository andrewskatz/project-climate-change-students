---
title: "q27 clustering and  climate beliefs"
author: "Katz"
date: "9/6/2020"
...
# alternative color scheme -- blues
cluster_colors <- colorRampPalette(c("#060a57", "#788FDD", "#ACE0EB"))(cluster_num)
...
cl_df %>% 
  select(student_id, Q16_vars) %>% 
  rename(energy_consump = Q16a,
         energy_efficiency = Q16b,
         reduce_CO2 = Q16c,
         sustainab_aware = Q16d,
         renew_tech = Q16e,
         infrast_resili = Q16f,
         clim_cha_leg = Q16g,
         comp_hum_intel = Q16h,
         ...
         cyber_threat = Q16k,
         drink_water = Q16l) %>% 
  pivot_longer(cols = -student_id, names_to = "Q16_item", values_to = "Q16_value") %>% 
  ggplot(aes(x = Q16_value)) +
  geom_histogram() +
  facet_wrap(Q16_item ~.)
...
cl_df %>% 
  select(student_id, major, Q16_vars) %>% 
  rename(energy_consump = Q16a,
         energy_efficiency = Q16b,
         reduce_CO2 = Q16c,
         sustainab_aware = Q16d,
         ...
         cyber_threat = Q16k,
         drink_water = Q16l) %>% 
  pivot_longer(cols = -c(student_id, major), names_to = "Q16_item", values_to = "Q16_value") %>% 
  ggplot(aes(x = Q16_value, y = major, fill = major)) +
  geom_boxplot() +
