---
title: "Q27 Full Info Clustering"
author: "Katz et al."
date: "5/17/2021"
...
# climate_imp_read <- readRDS(file = "climate_imp_40_2021-05-01.rds") # probably not what we want
...
data_file <- "climate_imp_long_40_full_inf_cl_hdb_agg_20210518.csv"
...
# [OLD]
# no longer need to drop NAs because they have been imputed
...
nested_imp <- nested_imp %>% 
  mutate(climate_27_umap = purrr::map(.x = climate_27_df, 
                                          ~ select(.x, all_of(Q27_vars)) %>%
...
nested_imp <- nested_imp %>% 
  mutate(plot_df = purrr::map(.x = climate_27_umap,
                       ~ as_tibble(.x$layout)))
...
```{r}
# write the full information umap by itself (without clustering)
today_date <- "2021-05-17"

write_rds(nested_imp, file = paste0("nested_imp_full_info_umap_", today_date, ".rds"))

```

### Finished with full info UMAP reduction



```{r}
nested_imp <- read_rds("nested_imp_full_info_umap_2021-05-17.rds")
```
...
## Clustering full info UMAP reduction
...
nested_imp <- nested_imp %>% 
  mutate(plot_df = purrr::map2(.x = plot_df, .y = hdbscan_umap_cl,
                              ~ mutate(.x, full_hdb_cluster = .y$cluster)))

# sensibility check
#nested_imp$plot_df[[1]]$cluster #check passed

# plot the reduction with the clusters for one figure
nested_imp$plot_df[[1]] %>% 
  ggplot(aes(x = dim1, y = dim2, color = as.factor(full_hdb_cluster))) +
  geom_point() +
...
purrr::map(.x = nested_imp$plot_df,
           ~ ggplot(.x, aes(x = dim1, y = dim2, 
                            color = as.factor(full_hdb_cluster))) +
             geom_point() +
             labs(x = "UMAP projection dimension 1", 
                  y = "UMAP projection dimension 2",
                  color = "Cluster",
...
nested_imp <- nested_imp %>% 
  mutate(climate_27_df = purrr::map2(.x = climate_27_df, .y = hdbscan_umap_cl,
                              ~ mutate(.x, full_hdb_cluster = .y$cluster)))
```
...
purrr::map(.x = nested_imp$climate_27_df,
           ~ ggplot(.x, aes(x = full_hdb_cluster)) +
             geom_histogram() +
             labs(x = "Cluster",
                  y = "Count",
...
agg_df <- nested_imp$plot_df[[1]] %>% select(dim1, dim2)
...
```{r}
umap_dist <- dist(agg_df, method  = "euclidean")
```


```{r}
hc1 <- hclust(umap_dist, method = "ward.D2")

```

```{r}
hc1_clusters <- cutree(hc1, k = 9)
table(hc1_clusters)
```
...
agg_df$full_agg_cluster <- hc1_clusters


agg_df %>% 
  ggplot(aes(x = dim1, y = dim2, color = as.factor(full_agg_cluster))) +
  geom_point() +
  labs(x = "UMAP projection dimension 1", 
       y = "UMAP projection dimension 2",
       color = "Cluster",
...
## conclusion: this could work as an alternative to using agnes
```




Clustering using agnes instead of hclust
...
# something weird is happening with this implementation of agglomerative clustering - it created a 10.5 GB file
```


```{r}
# trying alternative method for agglomerative clustering

# add option for agglomerative clustering instead of hdbscan

# this was a two step process, but probably not necessary to carry around the umap_dist matrix
# nested_imp <- nested_imp %>% 
#   mutate(umap_dist = purrr::map(.x = plot_df,
#                                 ~ select(.x, dim1, dim2) %>% 
#                                   dist(method  = "euclidean")),
#     agg_umap_cl = purrr::map(.x = umap_dist, 
#                                ~ hclust(.x, method = "ward.D2")))
```


```{r}
# try skipping the umap_dist creation since that is a huge matrix
nested_imp <- nested_imp %>% 
  mutate(agg_umap_cl = purrr::map(.x = plot_df,
                                ~ select(.x, dim1, dim2) %>% 
                                  dist(method  = "euclidean") %>% 
...
```{r}
today_date <- "2021-05-18"

# stopped writing this because it was still costing over 3GB
# write_rds(nested_imp, file = paste0("nested_imp_full_umap_agg_hdb_cl_", today_date, ".rds"))

# wrote to rds to save umap projects and agglomerative clusters since the clustering took so long

## at this point, hdbscan and agglomerative clustering have both been done. 
## the cluster assignments will be saved as full_hdb_cluster and full_agg_cluster 
```


```{r}
file_date <- "2021-05-18"
# nested_imp <- read_rds(paste0("nested_imp_full_umap_agg_hdb_cl_", file_date, ".rds"))
# still 3.5 GB to save the full nested_imp - probably just want to save the data csv
```
...
```{r}
## can use this block regardless of the agglomerative clustering method used (agnes or hclust)
...
Add the agglomerative clustering assignments back to the plot dataframe

```{r}

nested_imp <- nested_imp %>% 
  mutate(plot_df = purrr::map2(.x = plot_df, .y = agg_clusters,
                              ~ mutate(.x, full_agg_cluster = .y)))
...
Plot the results of agglomerative clustering to see the assignments

```{r}
# copied code from above, replacing hdb with agg
...
purrr::map(.x = nested_imp$plot_df,
           ~ ggplot(.x, aes(x = dim1, y = dim2, 
                            color = as.factor(full_agg_cluster))) +
             geom_point() +
             labs(x = "UMAP projection dimension 1", 
                  y = "UMAP projection dimension 2",
                  color = "Cluster",
...
# add the cluster labels back to the original data set
nested_imp <- nested_imp %>% 
  mutate(climate_27_df = purrr::map2(.x = climate_27_df, .y = agg_clusters,
                              ~ mutate(.x, full_agg_cluster = .y)))
```
...
nested_imp <- nested_imp %>% 
  mutate(data = purrr::map2(.x = data, .y = climate_27_df, 
                            ~ select(.y, c(.id, full_agg_cluster, full_hdb_cluster)) %>% 
                              inner_join(.x, by = ".id")))
...
## NOTE this is where the switch between agg and hdb comes in - calculating the cluster rankings

# for hdbscan
nested_imp <- nested_imp %>% 
  mutate(cluster_ranking_df_hdb = purrr::map(.x = data,
                                         ~ mutate(.x, 
                                                  Q27_total = Q27a +
                                                    Q27b + Q27c +
                                                    Q27d + Q27e +
                                                    Q27f + Q27g +
                                                    Q27h + Q27i) %>% 
                                           group_by(full_hdb_cluster) %>% 
                                           summarize(cluster_avg_hdb = mean(Q27_total)) %>% 
                                           arrange(cluster_avg_hdb) %>% 
                                           rowid_to_column(var = "cluster_time_rank_hdb")))

nested_imp <- nested_imp %>% 
  mutate(data = purrr::map2(.x = data, .y = cluster_ranking_df_hdb,
                            ~ inner_join(.x, .y, by = "full_hdb_cluster"))) 


# for agglomerative

nested_imp <- nested_imp %>% 
  mutate(cluster_ranking_df_agg = purrr::map(.x = data,
                                         ~ mutate(.x, 
                                                  Q27_total = Q27a +
                                                    Q27b + Q27c +
                                                    Q27d + Q27e +
                                                    Q27f + Q27g +
                                                    Q27h + Q27i) %>% 
                                           group_by(full_agg_cluster) %>% 
                                           summarize(cluster_avg_agg = mean(Q27_total)) %>% 
                                           arrange(cluster_avg_agg) %>% 
                                           rowid_to_column(var = "cluster_time_rank_agg")))

nested_imp <- nested_imp %>% 
  mutate(data = purrr::map2(.x = data, .y = cluster_ranking_df_agg,
                            ~ inner_join(.x, .y, by = "full_agg_cluster"))) 
nested_imp$data[[3]]
...
```{r}
# save the datasets stored in nested_imp$data to a csv to then import into each of the 
# individual analysis scripts

climate_imp_long <- nested_imp %>%
  select(.imp, data) %>% 
  unnest()

# climate_imp_long %>% write_csv("climate_imp_long_40_full_inf_cl_hdb_agg_20210518.csv")
...
# scale_num <- length(unique(climate_df$Q27a))
...
```{r}
# nested_imp$data[[1]]$cluster_time_
...
climate_df <- climate_27_df %>% select(student_id, cluster) %>% inner_join(climate_df, by = "student_id")
```
...
nested_imp <- nested_imp %>% 
  mutate(cluster_num_agg = map_int(.x = data, 
                               ~ length(unique(.x$cluster_time_rank_agg))))
...
# if climate_imp_long_cl is in name space, can use this:
climate_df <- climate_imp_long %>% filter(.imp == 1)

# if nested_imp is in name space, can use this:
...
```{r}
# using uwot implementation of umap


a <- 1.95
b <- 0.15

uwot_res <- uwot_umap_ab(climate_27_df, a = a, b = b)
...
custom.settings$n_components = 2
custom.settings$n_neighbors = 45 # next time, try dropping this from 55 to 30
custom.settings$min_dist = 0.000001 # experimental, dropping down from 0.1 to 0.0
custom.settings$metric = 'euclidean'



test_umap <- climate_27_df %>% 
  select(-student_id) %>% 
...
### Best combination: n_components=2, n_neighbords=80, min_dist=0.001, minPts=90 (or 50)
### also good: n_comp=2, n_neigh=20 or 30, min_dist=0.0001, minPts=50
hdbscan_umap_cl <- hdbscan(plot_df, minPts = 70) 
# groups stay pretty similar from minPts 70 up to 150, when it drops from 9 to 6 clusters
...
# add the cluster labels to the plotting dataframe
plot_df$cluster_hdb <- hdbscan_umap_cl$cluster

# plot the reduction with the clusters
plot_df %>% 
  ggplot(aes(x = dim1, y = dim2, color = as.factor(cluster_hdb))) +
  geom_point() +
  labs(x = "UMAP projection dimension 1", 
       y = "UMAP projection dimension 2",
       color = "Cluster",
...
```{r}
# add the cluster labels back to the original data set
climate_27_df$cluster_hdb <- hdbscan_umap_cl$cluster

climate_27_df %>% 
  ggplot(aes(x = cluster_hdb)) +
  geom_histogram() +
  labs(x = "Cluster",
       y = "Count",
       title = "Histogram of clusters for Question 27 ternary split") +
...
# if using directly from nested_imp
...
# trying clustering on the unreduced dataset (dataset without UMAP)
agg_df <- climate_27_df %>% select(-student_id)
...
```{r}
climate_27_df$cluster_agg <- sub_grp
```
...
# add the cluster labels to the plotting dataframe
plot_df$cluster_agg <- sub_grp

# plot the reduction with the clusters
plot_df %>% 
  ggplot(aes(x = dim1, y = dim2, color = as.factor(cluster_agg))) +
  geom_point() +
  labs(x = "UMAP projection dimension 1", 
       y = "UMAP projection dimension 2",
       color = "Cluster",
...
```{r}
# Add the cluster assignments back to the original climate_df dataframe
climate_df_join <- climate_27_df %>% select(student_id, cluster_hdb, cluster_agg) %>% inner_join(climate_df, by = "student_id")
```
...
climate_df_join %>% count(cluster_hdb, sort = TRUE)
...
climate_df_join %>% count(cluster_agg, sort = TRUE)
...
# decide whether to use cluster_agg or cluster_hdb

climate_df_join <- climate_df_join %>% 
  mutate(new_cluster = cluster_hdb)
...
## create dataframe with rankings for cluster by how soon each cluster believes climate change will affect groups
cluster_ranking_df <- climate_df_join %>% 
  mutate(Q27_total = Q27a + Q27b + Q27c + Q27d + Q27e +
           Q27f + Q27g + Q27h + Q27i) %>% 
  group_by(new_cluster) %>% 
  summarize(cluster_avg = mean(Q27_total)) %>% 
...
# new method of assigning cluster colors along a gradient
cluster_num <- length(unique(climate_df_join$new_cluster_time_rank))
