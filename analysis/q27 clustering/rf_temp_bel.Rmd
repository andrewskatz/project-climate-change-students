---
title: "Temporal Belief Patterns Random Forest"
author: "Katz et al."
date: "5/20/2021"
...
train_df %>% count(cluster_time_rank_agg)
...
rf_mod <- randomForest(formula = cluster_time_rank_agg ~ ., 
                       data = train_df,
                       ntree = 1000,
                       proximity = TRUE,
                       na.action = na.roughfix,
...
```{r}
train_df <- train_df %>%
  mutate(cluster_time_rank_agg = as.ordered(cluster_time_rank_agg)) 
...
ord_rf_df %>% select(cluster_time_rank_agg)

ord_rf_df <- as.data.frame(ord_rf_df)
# default mtry
rf_mod_ord <- ordfor(depvar = "cluster_time_rank_agg", 
                     data = ord_rf_df, 
...
Q20a -- strongly disagree to strongly agree
Q28_social_norm
...
climate_df <- climate_df %>% 
  mutate(Q20d_ord = case_when(Q20d == 0 ~ "Strongly disagree",
         ...
         Q20a_ord = case_when(Q20a == 0 ~ "Strongly disagree",
                              Q20a == 1 ~ "Disagree",
                              Q20a == 2 ~ "Neutral",
                              Q20a == 3 ~ "Agree",
                              Q20a == 4 ~ "Strongly agree"),
...
climate_df %>% count(Q20a)
climate_df %>% count(Q20a_ord)
...
rf_ord_vars <- c("Q20d_ord", "Q20a_ord", "Q26h_ord", "Q19h_ord", "Q20b_ord", "Q18k_ord")
...
```{r}
# no priors
mod1_ord_cl <- brm(formula = ordered(cluster_time_rank_agg) ~ Q28_tech_norm + Q20d + Q20a + Q28_social_norm + Q26h + Q19h + Q20b + Q18k,
                     data = climate_df,
...
```{r}
# no priors
mod1_lin_cl <- brm(formula = cluster_time_rank_agg ~ Q28_tech_norm + Q20d + Q20a + Q28_social_norm + Q26h + Q19h + Q20b + Q18k,
                     data = climate_df,
                     family = gaussian(),
                     cores = parallel::detectCores())
...
mod1_lin_center_cl <- brm(formula = cluster_time_rank_agg ~ Q28_tech_norm_center + Q20d_center + 
                            Q20a_center + Q28_social_norm_center + Q26h_center + 
...
file_name <- "mod1_lin_rf_cl.rds"

mod1_lin_cl %>% write_rds(file = file_name)
...
file_name <- "mod1_lin_cent_rf_cl.rds"

mod1_lin_center_cl %>% write_rds(file = file_name)

```

```{r}

mod1_lin_cl <- read_rds(file = file_name)

```

```{r}
tab_model(mod1_lin_center_cl)
```
...
```{r}
get_variables(mod1_lin_cl)
```


```{r}
stanplot(mod1_lin_cl, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
mod2_lin_mono_cl <- brm(formula = cluster_time_rank_agg ~ Q28_tech_norm + mo(Q20d_ord) + 
                            mo(Q20a_ord) + Q28_social_norm + mo(Q26h_ord) + 
...
file_name <- "mod2_lin_mono_rf_cl.rds"

mod2_lin_mono_cl %>% write_rds(file = file_name)
...
mod2_lin_mono_cl <- read_rds(file = file_name)

```

```{r}

get_variables(mod2_lin_mono_cl)
...
plot(conditional_effects(mod2_lin_mono_cl))
...
param_plot <- mod2_lin_mono_cl %>%
  ...
  rename(Q28_tech_norm = b_Q28_tech_norm,
         ...
         Q19h = bsp_moQ19h_ord,
         Q18k = bsp_moQ18k_ord)
  pivot_longer(cols = b_Q28_tech_norm:bsp_moQ18k_ord, names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  stat_halfeye(.width = c(.9, .5)) +
